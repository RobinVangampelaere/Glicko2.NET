...
namespace Glicko2
{
    public class RatingCalculator
    {
        ...
        public void UpdateRatings(RatingPeriodResults results)
        {
            ...
            {
                if (results.GetResults(player).Count > 0) CalculateNewRating(player, results.GetResults(player));
                else
                {
            ...
            foreach (var player in results.GetParticipants()) player.FinaliseRating();
        ...
        public double ConvertRatingToOriginalGlickoScale(double rating) => ((rating * Multiplier) + DefaultRating);
        ...
        public double ConvertRatingToGlicko2Scale(double rating) => ((rating - DefaultRating) / Multiplier);
        ...
        /// <returns></returns>
        public double ConvertRatingDeviationToOriginalGlickoScale(double ratingDeviation) => (ratingDeviation * Multiplier);
        ...
        public double ConvertRatingDeviationToGlicko2Scale(double ratingDeviation) => (ratingDeviation / Multiplier);
        public double GetDefaultRating() => DefaultRating;
        public double GetDefaultVolatility() => _defaultVolatility;
        public double GetDefaultRatingDeviation() => DefaultDeviation;
    }
}
